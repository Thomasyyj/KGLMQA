...
class GreaseLM_DataLoader(object):
    ...
    def __init__(self, train_statement_path, train_adj_path,
                 ...
                 subsample=1.0, n_train=-1, debug=False, cxt_node_connects_all=False, kg="cpnet", augmentation_times=2, nc=5):
        super().__init__()
        self.batch_size = batch_size
        ...
        ##--edit--
        self.shuffle_map = [[i for i in range(nc)] for j in range(augmentation_times)] # (augmentation_times, nc)
        random.seed(2023)
        ...
        *self.train_decoder_data, self.train_adj_data = self.load_sparse_adj_data_with_contextnode(train_adj_path, max_node_num, train_concepts_by_sents_list)
        ...
        # Load dev data
        self.dev_qids, self.dev_labels, self.dev_encoder_data, dev_concepts_by_sents_list = self.load_input_tensors1(dev_statement_path, max_seq_length)
        *self.dev_decoder_data, self.dev_adj_data = self.load_sparse_adj_data_with_contextnode1(dev_adj_path, max_node_num, dev_concepts_by_sents_list)
        if not debug:
        ...
        if test_statement_path is not None:
            self.test_qids, self.test_labels, self.test_encoder_data, test_concepts_by_sents_list = self.load_input_tensors1(test_statement_path, max_seq_length)
            *self.test_decoder_data, self.test_adj_data = self.load_sparse_adj_data_with_contextnode1(test_adj_path, max_node_num, test_concepts_by_sents_list)
            if not debug:
    ...
    def load_input_tensors(self, input_jsonl_path, max_seq_length):
        ...
        if use_cache:
            ...
            if self.model_type in ('lstm',):
                ...
                ##--edit--
                input_tensors = load_bert_xlnet_roberta_input_tensors(input_jsonl_path, max_seq_length, self.debug, self.tokenizer, self.debug_sample_size, self.shuffle_map)
                ##--edit--
            if not self.debug:
    ...
    def load_sparse_adj_data_with_contextnode(self, adj_pk_path, max_node_num, concepts_by_sents_list):
        ...
        ##---edit---
        (n_questions, num_choice, max_node_num) = concept_ids.size()
        # shuffle based on the rank of shuffle_map
        augmentation_times = len(self.shuffle_map)
        shuffle_map = torch.tensor(self.shuffle_map)
        concept_ids = concept_ids[:, shuffle_map, :].reshape(n_questions*augmentation_times, self.num_choice, max_node_num)
        node_type_ids = node_type_ids[:, shuffle_map, :].reshape(n_questions*augmentation_times, self.num_choice, max_node_num)
        node_scores = node_scores[:, shuffle_map, :].reshape(n_questions*augmentation_times, self.num_choice, max_node_num)
        adj_lengths = adj_lengths[:, shuffle_map].reshape(n_questions*augmentation_times, self.num_choice)
        special_nodes_mask = special_nodes_mask[:, shuffle_map, :].reshape(n_questions*augmentation_times, self.num_choice, max_node_num)
        edge_index = [[edge_index[m][i] for i in shuffle_map[j]] for j in range(augmentation_times) for m in range(n_questions)]
        edge_type = [[edge_type[m][i] for i in shuffle_map[j]] for j in range(augmentation_times) for m in range(n_questions)]
    ...
    def load_input_tensors1(self, input_jsonl_path, max_seq_length):
        """Construct input tensors for the LM component of the model."""
        cache_path = input_jsonl_path + "-sl{}".format(max_seq_length) + (("-" + self.model_type) if self.model_type != "roberta" else "") + '.loaded_cache'
        use_cache = True
        ...
        if use_cache:
            ...
            if self.model_type in ('lstm',):
                ...
                input_tensors = load_bert_xlnet_roberta_input_tensors1(input_jsonl_path, max_seq_length, self.debug, self.tokenizer, self.debug_sample_size)
            if not self.debug:
                utils.save_pickle(input_tensors, cache_path)
        return input_tensors

    def load_sparse_adj_data_with_contextnode1(self, adj_pk_path, max_node_num, concepts_by_sents_list):
        """Construct input tensors for the GNN component of the model."""
        print("Loading sparse adj data...")
        cache_path = adj_pk_path + "-nodenum{}".format(max_node_num) + ("-cntsall" if self.cxt_node_connects_all else "") + '.loaded_cache'
        use_cache = True
        ...
        shuffle_map=[0,1,2,3,4]
        shuffle_map = torch.tensor(shuffle_map)
        concept_ids = concept_ids[:, shuffle_map, :].reshape(n_questions, self.num_choice*augmentation_times, max_node_num)
        node_type_ids = node_type_ids[:, shuffle_map, :].reshape(n_questions, self.num_choice*augmentation_times, max_node_num)
        node_scores = node_scores[:, shuffle_map, :].reshape(n_questions, self.num_choice*augmentation_times, max_node_num)
        adj_lengths = adj_lengths[:, shuffle_map].reshape(n_questions, self.num_choice*augmentation_times)
        special_nodes_mask = special_nodes_mask[:, shuffle_map, :].reshape(n_questions, self.num_choice*augmentation_times, max_node_num)
        # edge_index = [[edge_index[m][i] for j in range(augmentation_times) for i in shuffle_map[j]] for m in range(n_questions)]
        # edge_type = [[edge_type[m][i] for j in range(augmentation_times) for i in shuffle_map[j]] for m in range(n_questions)]
        edge_index = [[edge_index[m][i] for i in shuffle_map] for m in range(n_questions)]
        edge_type = [[edge_type[m][i] for i in shuffle_map] for m in range(n_questions)]
...
def load_bert_xlnet_roberta_input_tensors(statement_jsonl_path, max_seq_length, debug, tokenizer, debug_sample_size, shuffle_map):
    ...
    class InputFeatures(object):
        ...
        def __init__(self, example_id, choices_features, label):
            ...
            self.choices_features = [
                ...
                for input_ids, input_mask, segment_ids, output_mask, sep_mask in choices_features
            ]
            self.label = label
    ...
    def simple_convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
        ...
        nc = len(examples[0].endings)
        answer_tokens = ["<ANS{}>".format(i) for i in range(nc)] 
        ...
        augmentation_times = len(shuffle_map)
        # shuffle_map_ = [i for i in shuffle_map for j in range(nc)] # (augmentation_times * nc, nc)
        ...
        for ex_index, example in tqdm(enumerate(examples), total=len(examples), desc="Converting examples to features"):
            if debug and ex_index >= debug_sample_size:
                break
            for aug_time in range(augmentation_times):
                choices_features = []
                labels=[]
                for t in range(nc):
                    ...
                    ans_mask = [int(i > mask_id) for i in input_ids]


                    # get the indices of all other occurrences of 2
                    indices_of_2 = [i for i, x in enumerate(input_ids) if x == 2]
                    ans_mask.append(indices_of_2[-1])
                    ...
                    choices_features.append((input_ids, input_mask, segment_ids, output_mask, ans_mask))
                    labels.append(label_map[shuffled_labels])
    ...
    def convert_features_to_tensors(features):
        ...
        all_sep_mask = torch.tensor(select_field(features, 'ans_mask'), dtype=torch.long)
        all_label = torch.tensor([f.label for f in features], dtype=torch.long)
        return all_input_ids, all_input_mask, all_segment_ids, all_output_mask, all_sep_mask, all_label
...
def load_bert_xlnet_roberta_input_tensors1(statement_jsonl_path, max_seq_length, debug, tokenizer, debug_sample_size):
    class InputExample(object):
    ...
    def simple_convert_examples_to_features1(examples, label_list, max_seq_length, tokenizer):
        """ Loads a data file into a list of `InputBatch`s
            `cls_token_at_end` define the location of the CLS token:
                - False (Default, BERT/XLM pattern): [CLS] + A + [SEP] + B + [SEP]
                - True (XLNet/GPT pattern): A + [SEP] + B + [SEP] + [CLS]
    ...
    examples = read_examples(statement_jsonl_path)
    features, concepts_by_sents_list = simple_convert_examples_to_features1(examples, list(range(len(examples[0].endings))), max_seq_length, tokenizer)
    example_ids = [f.example_id for f in features]
    *data_tensors, all_label = convert_features_to_tensors(features)
    return example_ids, all_label, data_tensors, concepts_by_sents_list
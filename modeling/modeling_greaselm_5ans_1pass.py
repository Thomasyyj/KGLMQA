...
class GreaseLM(nn.Module):
    ...
    def __init__(self, args={}, model_name="roberta-large", k=5, n_ntype=4, n_etype=38,
        ...
        super().__init__()
        if args.info_exchange == True:
            raise ValueError('In 5ans_1pass, info_exchange is not supported')
    ...
    def forward(self, *inputs, cache_output=False, detail=False):
        ...
        *lm_inputs, concept_ids, node_type_ids, node_scores, adj_lengths, special_nodes_mask, edge_index, edge_type = _inputs
        ...
        logits, attn, sent_vecs = self.lmgnn(nc,lm_inputs, concept_ids,
...
class LMGNN(nn.Module):
    ...
    def __init__(self, args={}, model_name="roberta-large", k=5, n_ntype=4, n_etype=38,
        ...
        if self.args.allow_graph is True:
            if 'cat' in self.args.last_layer_pooling_type or 'gate' in self.args.last_layer_pooling_type:
        ...
        #     case 'max':
        #         self.last_layer_pooling = CatPooling('max',False)
        #     case 'mean':
        #         self.last_layer_pooling = CatPooling('mean',False)
        #     case 'layer_wise_max':
        ...
        if self.args.last_layer_pooling_type =='max':
                self.last_layer_pooling = CatPooling('max',False)
    ...
    def forward(self, nc, inputs, concept_ids, node_type_ids, node_scores, adj_lengths, special_nodes_mask, adj, emb_data=None,
                cache_output=False):
        """
        ...
        input_ids = input_ids[::nc]
        attention_mask = attention_mask[::nc]
        token_type_ids = token_type_ids[::nc]
        output_mask = output_mask[::nc]
        ...
        if self.layer_wise_pooling is not None:
            ...
            sent_vecs = self.concat_pooling(hidden_states, input_ids, nc)
        else:
            hidden_states = all_hidden_states[self.layer_id]  # [bs, seq_len, sent_dim]
            sent_vecs = self.last_layer_pooling(hidden_states, input_ids, nc)
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if i >= self.num_hidden_layers - self.k:
                ...
                if self.info_exchange == True or (
                    ...
                    context_node_lm_feats = self.info_pooling(hidden_states, pooling_type)
...
class CatPooling(nn.Module):
    ...
    def forward(self, hidden_states, input_ids,nc):
        # hidden_states: [bs, seq_len, sent_dim]
        # input_ids: [bs, seq_len]
        bs = hidden_states.size(0)
        sep_span = [(torch.nonzero(input_ids[i] == 2)[-(nc+1):, :].squeeze()).unsqueeze(0) for i in range(bs)]
        sep_span = torch.cat(sep_span, dim=0)  # (bs, nc+1)

        cls_vec = hidden_states[:, 0, :]  # (bs, sent_dim)
        if self.mode == 'max':
            ans_vec = [torch.max(hidden_states[i, sep_span[i][j] + 1:sep_span[i][j+1], :], dim=0)[0].unsqueeze(0) for i in
                       range(bs) for j in range(nc)]
        elif self.mode == 'mean':
            ans_vec = [torch.mean(hidden_states[i, sep_span[i][j] + 1:sep_span[i][j+1], :], dim=0).unsqueeze(0) for i in
                       range(bs) for j in range(nc)]

        ans_vec = torch.cat(ans_vec, dim=0)  # bs*nc, embed_size
        if self.cat:
        # bs, embed_size*2
            return torch.cat([torch.repeat_interleave(cls_vec,nc,dim=0), ans_vec], dim=-1)
        return ans_vec
...
class GateInteraction(nn.Module):
    ...
    def forward(self, hidden_states, input_ids,nc):
        ...
        # input_ids: [bs*nc, seq_len]
        bs, seq_len, sent_dim = hidden_states.size()
        # print('truth:')
        # print(torch.mean(hidden_states[0,32:36,:], dim=1))

        sep_span = [(torch.nonzero(input_ids[i] == 2)[-(nc + 1):, :].squeeze()).unsqueeze(0) for i in
                    range(bs)]  # (bs, seq_len)
        sep_span = torch.cat(sep_span, dim=0)  # (bs, 6)
        sep_span = sep_span.unfold(1, 2, 1)  # (bs, 5, 2)

        if 'max' in self.mode:
            ans_vec = [torch.max(hidden_states[i, sep_span[i][j][0] + 1:sep_span[i][j][1], :], dim=0)[0].unsqueeze(0)
                       for i in range(bs) for j in range(nc)]
        elif 'mean' in self.mode:
            ans_vec = [torch.mean(hidden_states[i, sep_span[i][j][0] + 1:sep_span[i][j][1], :], dim=0).unsqueeze(0) for
                       i in range(bs) for j in range(nc)]

        # ans_vec: [bs*nc, sent_dim]
        ans_vec = torch.cat(ans_vec, dim=0)  # bs, embed_size
        sent_vecs = ans_vec.reshape(bs, nc, sent_dim)
        # print('get:')
        # print(sent_vecs[0,0,:])
        ...
        # sep_span = [(torch.nonzero(input_ids[i] == 2)[-(nc+1):, :].squeeze()).unsqueeze(0) for i in range(bs)]
        # sep_span = torch.cat(sep_span, dim=0)  # (bs, nc+1)
        # sep_span = sep_span[::nc,:]

        # cls_vec = hidden_states[::nc, 0, :]  # (bs, sent_dim)
        # bs = int(bs/nc)
        # if 'max' in self.mode:
        #     ans_vec = [torch.max(hidden_states[i, sep_span[i][j] + 1:sep_span[i][j+1], :], dim=0)[0].unsqueeze(0) for i in
        #                range(bs) for j in range(nc)]
        ...
        #                range(bs) for j in range(nc)]
        # ans_vec = torch.cat(ans_vec, dim=0)  # bs, embed_size
        # sent_vecs = ans_vec.view(bs, nc, -1).contiguous()
        ...
        for i in range(nc):
            ...
            H_outputs.append(H_out.unsqueeze(1))
        H_outputs = torch.cat(H_outputs, dim=1).reshape(bs*nc, sent_dim)  # bs*nc, hidden_dim

        if 'cat' in self.mode:
            cls_vec = hidden_states[:, 0, :]
            return torch.cat([H_outputs, torch.repeat_interleave(cls_vec, nc, dim=0)], dim=-1)  # bs*nc, hidden_dim*2

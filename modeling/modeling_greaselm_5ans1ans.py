...
class LMGNN(nn.Module):
    ...
    def forward(self, inputs, concept_ids, node_type_ids, node_scores, adj_lengths, special_nodes_mask, adj, emb_data=None, cache_output=False):
        ...
        ans_span = [(torch.nonzero(input_ids[i]==2)[-2:,:].squeeze()+torch.tensor([1,0], device='cuda:0')).unsqueeze(0) for i in range(bs)]
        ans_span = torch.cat(ans_span, dim=0) # (bs, 2)
        ans_vec = [torch.max(hidden_states[i, ans_span[i,0]:ans_span[i,1], :], dim=0)[0].unsqueeze(0) for i in range(bs)]
        # ans_vec = [torch.mean(hidden_states[i, ans_span[i,0]:ans_span[i,1], :], dim=0).unsqueeze(0) for i in range(bs)]
        ans_vec = torch.cat(ans_vec, dim=0) # bs, embed_size
        
        if self.args.kil is False:
            if self.args.allow_graph is True:
                concat = torch.cat((graph_vecs, sent_vecs, ans_vec, Z_vecs), 1)
            else:
                ...
                if self.args.allow_fuse==True:
                    concat = torch.cat((sent_vecs, ans_vec), 1)
                else: 
                    concat = ans_vec
        else:
            concat = torch.cat([sent_vecs, ans_vec], dim=1)
            diff = torch.abs(sent_vecs-ans_vec)
#             sent_vecs = sent_vecs.view(self.bs, self.nc, self.hidden_dim).contiguous()
#             H_outputs = []
...
#             if self.args.allow_fuse==True:
#                 concat = torch.cat((ans_vec, H_outputs), 1)
#             else: 
#                 concat = H_outputs
        if self.args.allow_fuse==True: 
            # if self.training:
            ...
            # else:
            #     logits = -F.cosine_similarity(sent_vecs, ans_vec, dim=1) 
            logits1 = self.proj(diff) 
            logits2 = self.fc(concat)
            if not training:
                logits = (logits1 + logits2)/2
                return logits, pool_attn, concat
            else:
                
        else:
            logits = self.proj(diff) 
            return logits, pool_attn, diff
            # logits = self.proj(concat) 
        # return logits, pool_attn, concat

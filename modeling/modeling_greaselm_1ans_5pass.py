...
class TextKGMessagePassing(ModelClass):
    ...
    def __init__(self, config, args={}, k=5, n_ntype=4, n_etype=38, dropout=0.2, concept_dim=200, ie_dim=200, p_fc=0.2, info_exchange=True, ie_layer_num=1, sep_ie_layers=False, pooling_type=None):
        ...
        self.encoder = RoBERTaGAT(config, k=k, n_ntype=n_ntype, n_etype=n_etype, hidden_size=concept_dim, dropout=dropout, concept_dim=concept_dim, ie_dim=ie_dim, p_fc=p_fc, info_exchange=info_exchange, ie_layer_num=ie_layer_num, sep_ie_layers=sep_ie_layers, pooling_type=pooling_type)
...
class RoBERTaGAT(modeling_bert.BertEncoder):
    ...
    def __init__(self, config, k=5, n_ntype=4, n_etype=38, hidden_size=200, dropout=0.2, concept_dim=200, ie_dim=200, p_fc=0.2, info_exchange=True, ie_layer_num=1, sep_ie_layers=False, pooling_type = None):
...
class LayerwisePooling(nn.Module):
    ...
    def forward(self, all_hidden_states, attention_mask):
        ...
        hidden_states = torch.concat([all_hidden_states[i].unsqueeze(1) for i in range(len(all_hidden_states))], dim=1).reshape(bs*layer_len, seq_len, sent_dim) #[bs*layer_len, seq_len, sent_dim]
...
class CatPooling(nn.Module):
    ...
    def forward(self, hidden_states, input_ids):
        ...
        if self.mode == 'max':
            ans_vec = [torch.max(hidden_states[i, sep_span[i][0]+1:sep_span[i][1], :], dim=0)[0].unsqueeze(0) for i in range(bs)]
        elif self.mode == 'mean':
            ans_vec = [torch.mean(hidden_states[i, sep_span[i][0]+1:sep_span[i][1], :], dim=0).unsqueeze(0) for i in range(bs)]
